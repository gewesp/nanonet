...
const unsigned int cpl::crypt::blowfish::scm_auiInitP[18] = {
...
const unsigned int cpl::crypt::blowfish::scm_auiInitS[4][256] = {
	//0
...
//
// Convert a vector by static_cast<>ing its elements.
//

template< typename D , typename S >
std::vector< D > const vector_static_cast( std::vector< S > const& v ) 
{ return std::vector< D >( v.begin() , v.end() ) ; }

} // anonymous namespace


//Constructor - Initialize the P and S boxes for a given Key
cpl::crypt::blowfish::blowfish(const buffer &ucKey, const block& roChain) : m_oChain0(roChain), m_oChain(roChain)
{ init( ucKey ) ; }

cpl::crypt::blowfish::blowfish
( std::vector< char > const& key , const block& roChain ) 
: m_oChain0(roChain), m_oChain(roChain)
{ init( vector_static_cast< unsigned char >( key ) ) ; }

  
cpl::crypt::blowfish::blowfish
( std::string const& key , const block& roChain ) 
: m_oChain0(roChain), m_oChain(roChain)
{ init( buffer( key.begin() , key.end() ) ) ; }
...
// Encoding and decoding of integers
buffer cpl::crypt::blowfish::encodeInt(unsigned long integer)
{
  buffer buf(8);
  double digit;

  // max for an unsigned long is 4294967295 = 2^32 = 256^4 => 4 bytes
  //          256^8 = 4722366482869645213696

  for (int i=0; i<=4; i++)
  {
    digit = std::floor(static_cast<double>(integer) / std::pow(256., i) );
    digit -= (std::floor(static_cast<double>(integer) / std::pow(256., i+1)) * 256.);

    buf[i] = static_cast<unsigned char>(digit);
  }
  
  return buf;
}

unsigned long cpl::crypt::blowfish::decodeInt(const buffer &buf)
{
  double integer = 0;
...
void cpl::crypt::blowfish::encrypt(block &b)
{
	unsigned int uiLeft = b.m_uil;
	unsigned int uiRight = b.m_uir;
	uiLeft ^= m_auiP[0];
...
void cpl::crypt::blowfish::decrypt(block &b)
{
	unsigned int uiLeft = b.m_uil;
	unsigned int uiRight = b.m_uir;
	uiLeft ^= m_auiP[17];
...
block cpl::crypt::blowfish::bytesToBlock(const buffer &bytes)
{ return ::cpl::crypt::vectorToBlock( bytes ) ; }


buffer cpl::crypt::blowfish::blockToBytes(const block &b)
{ return blockToVector< unsigned char >( b ) ; }
...
std::string cpl::crypt::blowfish::convertChar2Hex(const unsigned char ch)
{
  unsigned char byte[2];
	byte[0] = ch/16;
	byte[1] = ch%16;

  std::string szHex(2, '\0');
...
unsigned char cpl::crypt::blowfish::convertHex2Char(const unsigned char* const szHex)
{
	unsigned char rch = 0;
	for(int i=0; i<2; i++)
	{
...
buffer cpl::crypt::blowfish::hex2Char(const std::string &hexStr)
{
  int len = hexStr.length();
   
  if (len%2 != 0) len--;

  buffer charStr(len/2);
  ...
  bytes[2]=0;
    ...
    charStr[i/2] = convertHex2Char(bytes);
	}
