...
//
// TODO(maybe): Implement using lambdas?
//
...
// 
// Throw an exception of the form s + ": " + strerror_r( errnum ).
//
// If errnum is 0, use errno as errnum.
//
// As of 6/2020, uses strerror_r() and is hence thread-safe.
// 
...
std::string get_strerror_message( int const errnum ) ;
...
template<> struct auto_resource_traits< int > {
  ...
  static void initialize( int const ) {}
