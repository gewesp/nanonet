...
/// @return The sequence s converted to a std::string
/// @pre s is a sequence of char
template< typename S > std::string string_from_sequence( S const& s ) ;

/// @return The sequence converted to the given container, i.e.
/// C(begin(s), end(s))
///
/// Usage e.g.:
/// std::list<double> l;
/// convert_to<std::vector<double>>(l);
template< typename C, typename S > C convert_to( S const& s) ;
...
/// Safely check whether i is one before end.
/// \return true iff i + 1 == end
template< typename it >
inline bool next_is_end( it i , const it& end ) {
  if (i == end) {
    return false;
  }
  ++i;
