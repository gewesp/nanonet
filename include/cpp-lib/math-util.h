...
{ return symmetric_modulo( a , 2 * pi ) ; }
...
//
// If m > 0, then delta x - u is taken modulo m and centered around zero.
//
// The oscillation frequency is i sqrt( b^2/4 - a ), the damping exponent is
// a/2.  The system is critically damped when b^2/4 - a is zero.
//
...
//
// Solvers have a step<>() function template that advance the state one time
// step.  Model input must have been set and are assumed to be constant over dt.
// The step<>() function template accepts a function f().  The expression
...
// namely after the first output() call.  f() is passed a const M& m, and is 
// not allowed to modify variables!
//

//
// Euler solver.  Simple explicit Euler step.
//
...
//
// 4th order Runge-Kutta solver.  More precise than Euler, but not 
// much more stable.  Evaluates the model several times.
//
